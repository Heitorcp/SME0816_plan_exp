...
```{r}
df02 %>% group_by(trat) %>% summarise(n = n(),
                                     media = mean(y),
                                     variancia = var(y),
                                     desvio_padr = sd(y))
```
...
trat1 <- c(1,-1/3,-1/3,-1/3) #contraste para comparacao dos 4 grupos
trat2 <- c(0,1,-1/2,-1/2) #contraste para comparacao dos grupos B,C e D
trat3 <- c(0,0,1,-1) #contraste para a comparacao dos grupos C e D
...
Os valores do p-valor dos trës contrastes utilizados, nos indicam as seguintes conclusões:

* Existe uma diferença significativa nos contrastes formados por todos os grupos.

* O contraste formado pelos grupos (C,D) apresenta maior significância entre todos os outros.

Podemos concluir que o tratamento com ***BDZ*** é mais eficaz na redução do *FCR* por animal quando comparado ao uso de ***Anti-stress***.
